...
describe("Listing", async () => {
  ...
  beforeEach(async () => {
    ...
    // 1155 without 2981 allocation to lister
    ...
    // 721 without 2891 allocation to lister
    ...
    // 721 with 2981 allocation to lister
    let ERC1155WithERC2981Mock = await ethers.getContractFactory("ERC1155WithERC2981Mock");
    ...
    let ERC721WithERC2981Mock = await ethers.getContractFactory("ERC721WithERC2981Mock");
  ...
  it("lister is the owner of 10 tokenId 1s of 1155 with 2981 set up in before each", async () => {
    let tokenId = 1;
    let balanceOfLister = await erc1155With2981Mock.balanceOf(lister.address, tokenId);
    expect(balanceOfLister.toString()).to.be.equal("10")
  });
  ...
  it("lister can't list 10 tokenId 1s if not approved", async () => {
    ...
    await expect(
      multiplace.connect(lister).list(
        tokenAddr,
        tokenId,
        amount,
        unitPrice,
  ...
  it("lister can't list 5 tokenId 1s then another 5 tokenId 1s", async () => {
    ...
    await erc1155Mock.connect(lister).setApprovalForAll(multiplace.address, true);

    await multiplace.connect(lister).list(
      tokenAddr,
      tokenId,
      amount,
      unitPrice,
    ...
    await expect(multiplace.connect(lister).list(
      tokenAddr,
      tokenId,
      amount,
      unitPrice,
  ...
  it("can list ERC1155 with ERC2981 with ERC20", async () => {
    ...
    await erc1155With2981Mock.connect(lister).setApprovalForAll(multiplace.address, true);
  ...
  it("can list ERC721 with ERC2981 with ERC20", async () => {
    ...
    await erc721With2981Mock.connect(lister).setApprovalForAll(multiplace.address, true);
  ...
  it("fails if NFT does not support 165", async () => {
    let EmptyContractMock = await ethers.getContractFactory("EmptyContractMock");
  ...
  it("fails if nftAddress is not ERC721 or ERC1155, but supports ERC165", async () => {
    let MockThatOnlySupports165 = await ethers.getContractFactory("MockThatOnlySupports165");
  ...
  it("fails if tokenId is not owned by seller for 1155", async () => {
    ...
    await expect(multiplace.connect(notLister).list(
      tokenAddr,
      tokenId,
      amount,
      unitPrice,
  ...
  it("fails if tokenId of ERC721 is not approved for all by seller", async () => {
    ...
    let isApprovedForAll = await erc721Mock.connect(lister).isApprovedForAll(lister.address, multiplace.address);
  ...
  it("fails if tokenId of ERC1155 is not approved for all by seller", async () => {
    ...
    let isApprovedForAll = await erc1155Mock.connect(lister).isApprovedForAll(lister.address, multiplace.address);
    expect(isApprovedForAll).to.be.equal(false);
  ...
  it("fails if price is 0 for ERC721", async () => {
    ...
    await erc721Mock.connect(lister).setApprovalForAll(multiplace.address, true);
  ...
  it("Royalties are correctly set for ERC721 if listing supports ERC2981", async () => {
    ...
    let royaltyFrom721 = await erc721With2981Mock.royaltyInfo(tokenId, unitPrice);
    ...
    let royaltyFromMultiplace = await multiplace.getRoyalties(lister.address, tokenAddr, tokenId);
  ...
  it.skip("Royalties are correctly set for ERC1155 if listing supports ERC2981", async () => {});
  it.skip("Royalties are correctly set if listing does not support ERC2981, but ERC721 has owner", async () => {});
  it.skip("Royalties are correctly set if listing does not support ERC2981, but ERC1155 has owner", async () => {});
  it.skip("Royalties are correctly set if listing does not support ERC2981 and but ERC721 does not have an owner", async () => {});
  it.skip("Royalties are correctly set if listing does not support ERC2981 and but ERC1155 does not have an owner", async () => {});
  it.skip("allListings updated correctly if 3 ERC721 items are listed", async () => {});
